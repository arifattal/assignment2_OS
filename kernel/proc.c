...
fork(void)
{
  ...
  *(np_kt->trapframe) = *(kt->trapframe);

  // Cause fork to return 0 in the child.
  np_kt->trapframe->a0 = 0;
...
exit(int status)
{
  ...
  //this will set the killed flag for t and set the state to runnable
  //we might think that calling kthread_exit would make more sense, but calling exit
  //is only preformed by the thread itself. kthread_kill sets t's kill flag to 1 and state to runnable.
  //next time t get run time it will set it's state to zombie
...
sched(void)
{
  ...
  struct kthread *kt = mykthread();
  //if(!holding(&p->lock))
  if(!holding(&kt->lock)) //we added this condition
    panic("sched p->lock"); //look for this...
  if(mycpu()->noff != 1)
    panic("sched locks");
...
forkret(void)
{
  //printf("got to forkret\n");
  static int first = 1;
  struct kthread *kt = mykthread(); //we added the following two lines might not be needed
  release(&kt->lock);
...
void
wakeup(void *chan){
  //printf("got to wakeup\n");
  struct proc *p;
  struct kthread *kt;
  //struct kthread *running_kt = mykthread();
  for(p = proc; p < &proc[NPROC]; p++) {
    for (kt = p->kthread; kt < &p->kthread[NKT]; kt++){
      //if(kt != running_kt){
        acquire(&kt->lock);
